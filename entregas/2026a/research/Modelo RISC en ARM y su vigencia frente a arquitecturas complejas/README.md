...
- Mayor facilidad para implementar *pipeline*  
...
## Tabla fundamental: Pipeline en RISC vs CISC

El *pipeline* es uno de los conceptos más importantes para entender por qué RISC es eficiente.

| Etapa del Pipeline | RISC (ARM)                              | CISC (x86)                                      |
|--------------------|------------------------------------------|------------------------------------------------|
| Fetch              | Tamaño fijo → fácil de predecir          | Tamaño variable → más complejo                 |
| Decode             | Decodificación simple y rápida           | Decodificación compleja                        |
| Execute            | Operaciones simples                      | Puede implicar múltiples micro-operaciones     |
| Memory Access      | Separado claramente (load/store)         | Puede estar integrado en la instrucción        |
| Write Back         | Flujo uniforme                           | Flujo menos predecible                         |
| Eficiencia general | Pipeline más limpio y escalable          | Requiere traducción interna a micro-ops tipo RISC |

> Esta estructura uniforme permite que ARM implemente pipelines más eficientes y consuma menos energía.
